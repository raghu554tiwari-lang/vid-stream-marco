...
// Route DRM CDN & license requests through our /xp proxy so upstream Origin/Referer is used.
// Must be injected as the FIRST head script so it patches fetch/XHR BEFORE video-script.js runs.
const XPROXY_INJECT = String.raw`<script>(function(){try{var HOSTS=/(^|\.)(pw\.live|cloudfront\.net|penpencil\.co|penpencil\.xyz)$/i;function b64u(s){return btoa(unescape(encodeURIComponent(s))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');}function wrap(u){try{var url=new URL(u,location.href);if(!/^https?:$/.test(url.protocol))return u;if(url.origin===location.origin)return u;if(!HOSTS.test(url.hostname))return u;return location.origin+'/xp/'+b64u(url.toString());}catch(e){return u;}}var _f=window.fetch;window.fetch=function(input,init){try{if(typeof input==='string'){input=wrap(input);}else if(input&&input.url){var nu=wrap(input.url);if(nu!==input.url){input=new Request(nu,input);}}}catch(e){}return _f.call(this,input,init);};var XO=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){try{u=wrap(u);}catch(e){}return XO.apply(this,[m,u].concat([].slice.call(arguments,2)));};}catch(e){console.error('[xproxy]',e);}})();</script>`;
